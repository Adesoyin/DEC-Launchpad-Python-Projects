...
**Fields description and missing percentage** 
...
| **neighbourhood_group**            | Broader area or region grouping neighborhoods          | 100.00          | Entirely missing — will be derived using neighbourhood/geogragraphical data.        |
...
## Data Ingestion
API link of the dataset was gotten from the site for data ingestion. Check the `airbnb_listings_london.csv` file to see output of the extraction.

## Data Exploration
**Observation**

🔭 The dataset contains `18 columns` and `96,651 rows`, comprising 6 float64, 7 int64, and 5 string (object) data types.

🔭  The `licence and neighbourhood_group` columns are entirely null (100% missing). Hence, the licence field would be dropped as it adds no analytical value, while neighbourhood_group will be retained for further transformation.

🔭  The `host_name, last_review, and reviews_per_month` columns have missing values of approximately 26%, 26%, and 35% respectively—well above the 10% recommended threshold for missingness.

🔭  The `last_review` column was identified as an object (string) data type instead of date, requiring conversion for accurate time-based analysis.

🔭  No duplicate rows were found in the dataset, as confirmed by data[data.duplicated()], which returned 0 duplicates.
...
## Data Enrichment
To enhance the dataset and enable deeper analytical insights, additional fields were engineered to support more meaningful interpretations and decision-making:

1. `price_per_booking` — This field was derived by multiplying the  minimum_nights   a guest is required to stay `by the price of the listing`. It provides an estimate of the expected revenue per booking and can be aggregated to assess potential annual revenue per host or neighbourhood.

2. `availability_bucket` — A categorical field was created to group listings based on their yearly availability (availability_365):

    Rare: fewer than 100 days available per year

    Part-time: 100–300 days available

    Full-time: more than 300 days available

This classification helps identify how actively listings are available on the platform throughout the year, offering insights into host engagement and listing strategy.

## Data Analysis

1. Top 10 most expensive neighborhoods based on `average price`

An analysis of the ten most expensive neighbourhoods reveals a clear pricing gradient across London. Westminster, Kensington and Chelsea, Lambeth, and the City of London top the list, with `average minimum night p`rices ranging between `$340 and $380, highlighting their premium accommodation demand and central location appeal. `Camden and Islington` fall into a moderate pricing band of $218–$230, suggesting balanced demand with a mix of affordability and accessibility. In contrast, Brent, Richmond upon Thames, Wandsworth, and Hammersmith & Fulham maintain relatively lower rates between $165 and $200, reflecting more residential or suburban market dynamics compared to central zones.
